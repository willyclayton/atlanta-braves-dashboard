...
// Also add countdown, history, and trivia to the initialization
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(updateBravesTrivia, 750); // Load Braves trivia
});

// New Trivia Functions - Added without modifying existing functions
async function fetchBravesTrivia() {
    try {
        // Fetch trivia from the JSON file generated by GitHub Actions
        const response = await fetch('./data/daily-trivia.json');
        
        if (!response.ok) {
            throw new Error(`Failed to fetch trivia data: ${response.status}`);
        }
        ...
        return {
            ...
            date: triviaData.date
        };
    } catch (error) {
        console.error('Error fetching trivia from JSON file:', error);
        // Fallback to a static verified fact
        return {
...
async function updateBravesTrivia() {
    try {
        const triviaData = await fetchBravesTrivia();
        
        // Update the question
        const questionElement = document.getElementById('trivia-question-text');
        if (questionElement) {
            questionElement.textContent = triviaData.question;
        }
        
        // Update the answer (but keep it hidden)
        const answerElement = document.getElementById('trivia-answer-text');
        if (answerElement) {
            answerElement.textContent = triviaData.answer;
        }
        
        // Make sure the answer is hidden initially
        const answerContainer = document.getElementById('trivia-answer');
        if (answerContainer) {
        ...
        const questionElement = document.getElementById('trivia-question-text');
        if (questionElement) {
            questionElement.textContent = "What historic achievement did Hank Aaron accomplish on April 8, 1974?";
        }
        
        const answerElement = document.getElementById('trivia-answer-text');
        if (answerElement) {
            answerElement.textContent = "Hank Aaron broke Babe Ruth's all-time home run record by hitting his 715th home run at Atlanta-Fulton County Stadium.";
        }
